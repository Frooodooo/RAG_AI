...
## 2024-05-25 - ARIA Labels on Implicitly Named Elements
**Learning:** Some elements in the UI like search inputs or dynamic close buttons rely entirely on placeholders or surrounding context, leaving screen readers without a clear label.
**Action:** Always provide an explicit `aria-label` for inputs without a visible `<label>` (like search boxes) and icon-only buttons (like toasts or dismiss alerts) to ensure the intent is conveyed.
