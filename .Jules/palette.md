...
## 2024-05-25 - Focus Management on Conditional Elements
**Learning:** Elements that conditionally render based on state (like the 'clear search' button) cause focus loss when they unmount, resetting focus to the body.
**Action:** When removing an interactive element that had focus, programmatically shift focus to a logical nearby element (e.g., the input field it cleared) using `useRef` and `useEffect` or the event handler.
