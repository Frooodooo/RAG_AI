...
## 2024-05-25 - Focus rings on Toast/Alert Close Buttons
**Learning:** When adding focus states (`focus-visible:outline`) to icon-only "close" buttons (like `×` inside an error toast), the default outline can look boxy and unpolished if the button lacks a defined `border-radius`.
**Action:** Always ensure icon-only buttons have an explicit `border-radius` (e.g., `var(--r-sm)`) when applying focus rings, even if they appear as plain text/icons in their default resting state.
