...
## 2025-02-14 - ARIA Labels for Icon-Only Buttons and Inputs
**Learning:** Inputs without visible labels (like search bars with placeholders) and icon-only buttons (like download/delete) must have explicit `aria-label`s for screen reader accessibility, and focus visible outlines.
**Action:** Always provide `aria-label` attributes using the `t()` function for localization, and use `focus-visible:outline` to ensure keyboard navigability without conflicting with inline styles.
