...
## 2026-03-12 - Memoizing Conditional Operations
**Learning:** When a component needs to memoize an expensive operation (like parsing Markdown) but also has an early return (e.g., if the message is from the user), placing `useMemo` after the early return violates React's Rules of Hooks.
**Action:** To fix the linting error without losing the performance optimization, move the hook above the early return and conditionally execute the expensive operation inside the hook's callback using a ternary operator (e.g., `useMemo(() => condition ? null : expensiveOp(), [condition])`).
