...
## 2024-05-25 - LocalStorage inside React State Setters
**Learning:** Calling synchronous blocking operations like `localStorage.setItem` directly inside React `setState` functional updates causes the React main thread to stall during render phases, and leads to duplicate writes in StrictMode.
**Action:** Always synchronize React state to `localStorage` or other synchronous storage mediums via `useEffect` hooks, keeping the state setter functions pure and non-blocking.
