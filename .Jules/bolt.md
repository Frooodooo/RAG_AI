...
**Learning:** In React Flow components (e.g., `WorkflowVisualizer`), when polling for status updates, unconditionally mapping and returning new node objects (`{ ...node, data: { ...node.data, status } }`) causes cascading re-renders of the entire flow, even if the actual status value hasn't changed.
**Action:** Conditionally return the exact original node object reference in the `map` callback if its data hasn't changed, preserving React Flow's internal memoization.
