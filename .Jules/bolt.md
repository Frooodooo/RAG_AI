...
## 2024-03-03 - [React String Conversion Bottlenecks]
**Learning:** Performing `search.toLowerCase()` inside a `.filter` or `.map` loop callback forces an O(N) redundant string conversion on every iteration during React renders.
**Action:** Always extract invariant transformations (like lowercasing the search query) outside the loop and wrap the entire calculation in `useMemo` to prevent unnecessary work on component re-renders.
