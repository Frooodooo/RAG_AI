...
## 2024-05-25 - React State Setter Side Effects
**Learning:** In `rag-demo/src/hooks/useChatSessions.ts`, calling blocking operations like `localStorage.setItem` synchronously inside React state setter functional updates (e.g., `setSessions((prev) => { ... localStorage.setItem(...) return next })`) blocks the React main thread and causes duplicate writes during StrictMode's double-invocation.
**Action:** Synchronize state to `localStorage` using a `useEffect` hook instead of embedding side effects directly in functional state updates.
