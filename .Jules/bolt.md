...
## 2024-05-25 - Sync Side-effects in React Setters
**Learning:** Calling blocking synchronous operations like `localStorage.setItem` directly inside React state setter callbacks (e.g., `setSessions(prev => ...)`) blocks the main thread during render and triggers duplicate writes in StrictMode.
**Action:** Move side-effects like `localStorage.setItem` outside of state setter callbacks, either by executing them sequentially or declaratively via `useEffect`.
